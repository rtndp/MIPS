	...
	private static void parseConfigurationLine(String line) {
		...
		case "fp multiplier":
			...
			ConfigManager.instance.FPMultLatency = Integer.parseInt(s1[0]
					.trim());
			ConfigManager.instance.FPMultPipelined = s1[1].trim().toLowerCase()
					.equals("yes");
			break;
		...
		case "main memory":
			ConfigManager.instance.MemoryLatency = Integer
					.parseInt(s[1].trim());
			break;

		case "i-cache":
			ConfigManager.instance.ICacheLatency = Integer
					.parseInt(s[1].trim());
			break;

		case "d-cache":
			ConfigManager.instance.DCacheLatency = Integer
					.parseInt(s[1].trim());
			break;
		}
